...
SlashCmdList["GRIMFALLHELPER"] = function(msg)
    ...
    elseif cmd == "plates" or cmd == "nameplates" or cmd == "debuffs" then
        local cur = GH.Config:Get("nameplateDebuffs", true)
        GH.Config:Set("nameplateDebuffs", not cur)
        Utils:Print("Enemy Nameplate Debuff Tracking set to " .. (not cur and "|cff00FF96ON|r" or "|cffFF3333OFF|r"))
    elseif cmd == "help" then
        ...
        DEFAULT_CHAT_FRAME:AddMessage("  |cff00FF96/gf plates|r - Toggle player debuffs/DoTs on enemy nameplates")
...
EventBus:Register("ADDON_LOADED", function(event, addonName)
    if string.lower(addonName) == "grimfallhelper" or addonName == C.ADDON_NAME then
        ...
        if GH.NameplateAuras then
            GH.NameplateAuras:Initialize()
        end
