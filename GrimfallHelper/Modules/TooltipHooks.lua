...
-- Universal GameTooltip Hooks for Classless Metadata, Wishlist Tiers, & Synergies
...
local function EnhanceTooltipWithSpell(tooltip, spellName)
    ...
    -- 2. Wishlist Status
    if GH.Wishlist then
        local tier = GH.Wishlist:GetTier(spellName)
        if tier then
            local tInfo = GH.Wishlist.TIERS[tier] or { label = tier .. "-Tier", color = "FFD700" }
            local isAcquired = GH.Wishlist:IsAcquired(spellName)
            local statusTag = isAcquired and "|cff00FF96[Learned]|r" or "|cffFFD700[Hunting Target]|r"

            if not addedLine then tooltip:AddLine(" ") addedLine = true end
            tooltip:AddDoubleLine(
                "Wishlist Priority:",
                "|cff" .. tInfo.color .. tInfo.label .. "|r " .. statusTag,
                0.8, 0.8, 0.8, 1, 1, 1
            )
        end
    end
